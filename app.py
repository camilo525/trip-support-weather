...
def generate_client_text(wx, icao, is_departure=True):
    ...
    if is_critical:
        return f"The Operations Team is currently analyzing weather conditions for your {role} {icao} and we are reporting if there are any issues. We are evaluating the most efficient window for your day of travel."
    return f"Meteorological analysis for your {role} {icao} indicates ideal conditions. The Operations Team is monitoring conditions and confirms a seamless process."

def get_coords(wx):
    for key in ['station', None]:
        try:
            target = wx[key] if key else wx
            return target['geometry']['coordinates'][1], target['geometry']['coordinates'][0]
...
with col_l: st.image(LOGO_UP_LEFT, width=300)
with col_r: st.markdown('<div class="header-style">Flight Support Team

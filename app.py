...
with st.sidebar:
    ...
    etapa = st.selectbox("FLIGHT MILESTONE (Etapa del Vuelo)", [
    ...
    etd = st.text_input("ETD (UTC)", value="12:00")
    eta = st.text_input("ETA (UTC)", value="16:00")
    tail = st.text_input("TAIL NUMBER", value="N123VIP")
...
def generate_client_text(wx, icao, type="dep"):
    if not wx: return "Weather monitoring is active for your window."
    raw = wx.get("raw_text", "").upper()
    vis = wx.get("visibility", {}).get("miles_float", 10)
    is_crit = any(x in raw for x in ["TS", "SN", "FG"]) or vis < 3
    if type == "dep":
        return f"Conditions at {icao} are favorable for departure." if not is_crit else f"Monitoring weather activity at {icao} for departure."
    return f"Skies are clear for arrival at {icao}." if not is_crit else f"Tracking local activity at {icao} for arrival."
...
def generate_vip_card(etapa, origin, destination, etd, eta, tail, wx_org, wx_dst):
    ...
    if "1." in etapa:
        ...
        main_msg = f"El avión <b>{tail}</b> está iniciando su reposicionamiento hacia {origin}. Todo marcha según lo previsto para su salida."
        weather_section = f"""<div style='background:#f9f9f9; padding:15px; border-radius:10px; margin-top:15px; text-align:left;'>
                                <p style='color:#555; font-size:13px; margin:0;'><b>Weather Departure:</b> {dep_w}</p>
                                <p style='color:#555; font-size:13px; margin:5px 0 0 0;'><b>Weather Arrival:</b> {arr_w}</p>
                              </div>"""
    elif "4." in etapa:
        ...
        main_msg = f"Iniciando rodaje en {origin}. El tiempo estimado de llegada es {eta}Z. Nuestro equipo de Flight Support estará monitoreando su ruta en tiempo real hasta su destino."

    return f"""
    <div style="font-family: Arial, sans-serif; max-width: 550px; border: 1px solid #eee; border-radius: 15px; overflow: hidden; background-color: #fff; margin: auto;">
        ...
        <div style="padding: 25px; text-align: center;">
            ...
            <p style="color: #444; line-height: 1.6; font-size: 15px; text-align: left; margin: 0;">{main_msg}</p>
            {weather_section}
        </div>
        <div style="background-color: #000; padding: 15px; text-align: center; color: #666; font-size: 10px; letter-spacing: 1px;">
...
if st.button("Generate VIP Milestone Briefing"):
    ...
    st.info("💡 **INSTRUCCIONES:** Sombrea la tarjeta de arriba con el ratón, cópiala y pégala en tu hilo de Gmail.")

# --- FOOTER ---
st.markdown("---")
st.markdown(f'<div class="footer-container"><img src="{LOGO_BOTTOM_CENTER}" width="160"></div>', unsafe_allow_html=True)
